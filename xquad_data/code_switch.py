...
See original license and terms of use in the CodeSwitchCLIR repository.
"""
import logging
import json
import os
...
parser.add_argument("--input_file", help="Input jsonl file to be code-switched", required=True)
parser.add_argument("--path_embeddings", help="Folder containing embedding files", required=True)
...
def load_embs(lang:str):
    ...
    for line in lines:
        ...
        if 'مالكوفيتش'== term: # skip corrupt line
            continue
        assert vec.size == 300
        vocab.append(term)
...
def code_switch(input_file: str, qlang: str, dlang: str, prob: float) -> None:
    """Apply code-switching to the input file"""
    random.seed(1)
    tgt_dir = join(tgt_basedir, f"translation_prob={prob}/{qlang + dlang}")
    output_file = join(tgt_dir, "train.jsonl")

    if os.path.exists(output_file):
        logger.info(f"{output_file} exists already, skipping.")
        return
    else:
        logger.info(f"Target file: {output_file}")

    # Load vocabulary and records
    corpus_vocab, query_vocab, records = load_data(input_file)
    ...
    # # Load translation tables for queries
    # query_translation_table = None
    # lang2query_ttables = None
    # if qlang == TOKEN_ANY:
    #     lang2query_ttables = {
    #         qlang: get_translation_table_from_knn(qlang, query_vocab, en_emb, en_vocab, "train", "query")
    #         for qlang in mapping.keys()
    #         if qlang != "en"
    #     }
    # else:
    #     query_translation_table = get_translation_table_from_knn(
    #         qlang, query_vocab, en_emb, en_vocab, "train", "query"
    #     ) if qlang != "en" else None

    # Load translation tables for documents
    doc_translation_table = None
    lang2doc_ttables = None
    if dlang == TOKEN_ANY:
        lang2doc_ttables = {
            dlang: get_translation_table_from_knn(dlang, corpus_vocab, en_emb, en_vocab, "train", "corpus")
            for dlang in mapping.keys()
            if dlang != "en"
        }
    else:
        doc_translation_table = get_translation_table_from_knn(
            dlang, corpus_vocab, en_emb, en_vocab, "train", "corpus"
    ...
    with open(output_file, "w") as f:
        for record in tqdm.tqdm(records):
            ...
            if dlang != "en":
                ...
                record["passage"] = detokenizer.detokenize(cs_tokens)

            # Remove tokenized versions
            del record["dtokens"]
            # del record["qtokens"]
    ...
    logger.info(f"{qlang}-{dlang}:\tDoc cs (no cs): {doc_cs_count} ({doc_nocs_count})")

def load_data(input_file: str):
    """Load data from jsonl file"""
    cache_file = join(cache_dir, "data_cache.pkl")
    ...
    if not os.path.exists(cache_file):
        ...
        logger.info("Re-loading cached data from %s" % cache_file)
...
def get_translation_table_from_knn(language:str, query_or_corpus_vocab:List[str], en_emb:np.array, en_vocab:List[str], train_or_dev:str, query_or_corpus:str):
    """Get translation table using KNN"""
...
def main():
    # Only use multilingual code-switching (ML-CS)
    for prob in [0.5]:
        logger.info(f"Translation probability: {prob}")
        
        qlang = "en" 
        dlang = TOKEN_ANY
        logger.info(f"Code-switching {qlang}-{dlang} (Multilingual)")
        code_switch(input_file, qlang, dlang, prob=prob)
    
        qlang = TOKEN_ANY
        dlang = TOKEN_ANY
        logger.info(f"Code-switching {qlang}-{dlang} (Dual-side ML-CS)")
        code_switch(input_file, qlang, dlang, prob=prob)
