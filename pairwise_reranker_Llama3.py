...
class PairwiseLlmRanker_HF(LlmRanker):
    def __init__(self,
        ...
        self.activation_records_by_layer = {i: [] for i in range(self.model.config.num_hidden_layers + 1)}
    ...
    def _get_response(self, input_text, probe=False):
        message = [
        ...
        if probe == True:
            if hidden_states:
                for layer_idx, hidden in enumerate(hidden_states):
                    residual = hidden[0, -1, :].cpu()  # shape: [D]
                    self.activation_records_by_layer[layer_idx].append(residual)
    ...
    def save_all_layerwise_activations(self, save_dir):
        os.makedirs(save_dir, exist_ok=True)
        for layer_idx, residuals in self.activation_records_by_layer.items():
            data = {
                "activations": torch.stack(residuals)  # shape [N, D]
            }
            torch.save(data, os.path.join(save_dir, f"layer_{layer_idx}.pt"))
            self.logger.info(f"Saved {len(residuals)} samples to layer_{layer_idx}.pt")
